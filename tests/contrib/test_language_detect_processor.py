...
async def test_detect_english():
  ...
  part = ProcessorPart(
      "This is a test in English language.", mimetype="text/plain", metadata={}
  )
...
async def test_detect_french():
  ...
  processor = LanguageDetectProcessor()
  part = ProcessorPart(
      "Bonjour, comment allez-vous? Ceci est un texte en français.",
      mimetype="text/plain",
      metadata={},
  )
...
async def test_detect_bengali():
  ...
  processor = LanguageDetectProcessor()
  part = ProcessorPart(
      "আমি বাংলায় কথা বলি। এটি একটি বাংলা পাঠ্য।",
      mimetype="text/plain",
      metadata={},
  )
...
async def test_short_text_unknown():
  ...
  part = ProcessorPart("Hi", mimetype="text/plain", metadata={})
...
async def test_empty_text_unknown():
  ...
  processor = LanguageDetectProcessor()
  part = ProcessorPart("", mimetype="text/plain", metadata={})
...
async def test_non_text_part_unchanged():
  ...
  processor = LanguageDetectProcessor()
  part = ProcessorPart(
      b"fake_image_data", mimetype="image/jpeg", metadata={"some": "data"}
  )
...
async def test_multiple_parts():
  ...
  processor = LanguageDetectProcessor()
  parts = [
      ProcessorPart("This is English.", mimetype="text/plain", metadata={}),
      ProcessorPart("Bonjour le monde.", mimetype="text/plain", metadata={}),
      ProcessorPart(b"image_data", mimetype="image/png", metadata={}),
      ProcessorPart("这是中文。", mimetype="text/plain", metadata={}),
  ]

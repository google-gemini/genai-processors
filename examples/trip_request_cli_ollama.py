...
async def run_trip_request() -> None:
  ...
  generate_trip = ollama_model.OllamaModel(
      # NOTE: For the demo we use a small 4B Gemma model which is fast and
      # doesn't require a lot of RAM. The real application would use a better
      # but slower model. It also may mix local Gemma for TripRequest extraction
      # and Gemini Pro for generating the trip plan.

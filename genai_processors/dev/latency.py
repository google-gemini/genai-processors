...
class ProbeCheckpoint(processor.Processor):
  ...
  def __init__(self, tag: str, substream_name: str = ''):
    ...
    Args:
        ...
        substream_name: Only parts in this substream are considered to compute
          the metrics and to add the next marker to the probe info. Default
          substream is used if empty string.
    """
    self.tag = tag
...
class ProbeLog(processor.Processor):
  ...
  def __init__(
    ...
    Args:
        substream_name: Parts will be output on this substream. Note that parts
          from all substreams are considered.
        part_formatter: A function that returns the string representation of a
          part. Default is `None`: no representation is shown during latency
          trace. If set, the log will include the part representation after the
          latency trace. This can be useful for debugging.
        latency_substream: The name of the substream to emit the latency part
          on. Default is "latency". When set to `None`, no latency part is
          emitted.
    """
...
class Throttler(processor.Processor):
  ...
  def __init__(
      self, tag: str, max_size: int = 2, log_interval_sec: float = 5.0
  ):
  ...
  async def call(
    ...
    drop_buffer: collections.deque[content_api.ProcessorPart] = (
        collections.deque(maxlen=self.max_size)
    )

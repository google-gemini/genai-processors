...
class TraceTest(unittest.IsolatedAsyncioTestCase):
  ...
  async def test_trace_generation_and_timestamps(self):
    ...
    self.assertEqual(
        root_trace.parts_store[sub_trace.events[1].part_ref]['part']['text'],
        'HELLO_sub_trace',
    )
    ...
    self.assertEqual(
        root_trace.parts_store[trace.events[1].part_ref]['part']['text'],
        'hello',
    )
    self.assertFalse(trace.events[2].is_input)
    self.assertIn(
        'TEST_SUB_PROCESSOR',
        root_trace.parts_store[trace.events[2].part_ref]['part']['text'],
    )
    self.assertFalse(trace.events[3].is_input)
    self.assertEqual(
        root_trace.parts_store[trace.events[3].part_ref]['part']['text'],
  ...
  async def test_image_resizing(self):
    ...
    self.assertIsNotNone(event.part_ref)

    part_dict = trace.parts_store[event.part_ref]
